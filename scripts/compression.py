"""Tiered memory compression engine â€” age-based, zero-LLM, entity-density scoring.
    ...
    for match in candidates:
        ...
        if words:
            ...
            if last_word in _ABBREVIATIONS:
                # Not a real sentence boundary â€” skip.
                continue
        if fragment:
...
class CompressionEngine:
    ...
    def determine_target_tier(
        ...
        *created_at*). The current *compression_tier* is not taken into account
        â€” callers should compare this value against ``fiber.compression_tier``
    ...
    async def compress_fiber(
        ...
        *,
        ...
        if target_tier == CompressionTier.GRAPH_ONLY:
            ...
            # FULL â€” no compression needed (should be filtered before here).
            return CompressionResult(
                fiber_id=fiber.id,
                original_tier=fiber.compression_tier,
                new_tier=fiber.compression_tier,
...
# ---------------------------------------------------------------------------
# InfinityNeural: 3-Tier Compression System
# ---------------------------------------------------------------------------


class InfinityTier(IntEnum):
    """InfinityNeural 3-tier compression system.
    
    Unlike the original 5-tier age-based system, InfinityNeural uses
    a hybrid approach: age + frequency + neuron_type to determine tier.
    
    Tier 0 (HOT):   Full detail, < 30d OR frequency > 5 OR ETERNAL
    Tier 1 (WARM):  Summary + key neurons, 30-180d, frequency 1-5
    Tier 2 (COLD):  Core facts only, > 180d, frequency 0-1
    """
    
    HOT = 0   # Full detail
    WARM = 1  # Summary + key neurons
    COLD = 2  # Core facts only


@dataclass(frozen=True)
class InfinityTierConfig:
    """Configuration for InfinityNeural tier system."""
    
    # Age thresholds (days)
    hot_max_age_days: float = 30.0      # < 30d = HOT
    warm_max_age_days: float = 180.0    # 30-180d = WARM, > 180d = COLD
    
    # Frequency thresholds
    hot_min_frequency: int = 5          # frequency > 5 = always HOT
    warm_min_frequency: int = 1         # frequency 1-5 = eligible for WARM
    
    # Tier 1 (WARM) compression settings
    warm_keep_neuron_types: frozenset[str] = field(
        default_factory=lambda: frozenset({"entity", "concept", "eternal", "anchor"})
    )
    
    # Tier 2 (COLD) compression settings
    cold_keep_neuron_types: frozenset[str] = field(
        default_factory=lambda: frozenset({"entity", "concept", "eternal"})
    )
    cold_keep_synapse_types: frozenset[str] = field(
        default_factory=lambda: frozenset({"caused_by", "leads_to", "is_a"})
    )


class InfinityTierManager:
    """InfinityNeural tier manager â€” 3-tier compression with relevance preservation.
    
    Unlike original CompressionEngine which prunes old content, InfinityNeural
    NEVER deletes â€” only compresses to lower tiers. All memories remain recoverable.
    """
    
    def __init__(
        self,
        storage: NeuralStorage,
        config: InfinityTierConfig | None = None,
    ) -> None:
        self._storage = storage
        self._config = config or InfinityTierConfig()
    ...
    def determine_target_tier(
        ...
        """Determine target tier using age + frequency + neuron types.
        
        Rules:
        - ETERNAL neurons â†’ always HOT
        - Age < 30d OR frequency > 5 â†’ HOT
        - Age 30-180d AND frequency 1-5 â†’ WARM
        - Age > 180d AND frequency 0-1 â†’ COLD
        - ANCHOR neurons â†’ never COLD (cap at WARM)
        ...
        # Check if fiber contains ETERNAL neurons (always HOT)
        # Note: This requires loading neurons, expensive but necessary
        # In practice, ETERNAL check should be done at fiber metadata level
        # For now, use frequency + age heuristic
        
        # Rule 1: High frequency â†’ always HOT
        if frequency > self._config.hot_min_frequency:
            return InfinityTier.HOT
        
        # Rule 2: Recent â†’ HOT
        if age_days < self._config.hot_max_age_days:
            return InfinityTier.HOT
        
        # Rule 3: Old + low frequency â†’ COLD
        if age_days > self._config.warm_max_age_days and frequency <= 1:
            return InfinityTier.COLD
        
        # Default: WARM
        return InfinityTier.WARM
    
    async def demote_fiber(
        self,
        fiber: Fiber,
        target_tier: InfinityTier,
        *,
        dry_run: bool = False,
    ) -> CompressionResult:
        """Demote (compress) fiber to target tier.
        
        InfinityNeural compression preserves structure:
        - HOTâ†’WARM: Create summary, compress ACTION/STATE/SENSORY neurons
        - WARMâ†’COLD: Keep only ENTITY + CONCEPT + ETERNAL, core synapses
        
        Args:
            fiber: Fiber to compress
        ...
        current_tier = InfinityTier(fiber.compression_tier)
        ...
        if target_tier <= current_tier:
            return CompressionResult(
                ...
                backup_created=False,
                skipped=True,
                skip_reason="already_at_or_beyond_target",
            )
        
        # Fetch neurons
        neurons = await self._storage.get_neurons_batch(list(fiber.neuron_ids))
        neuron_contents = [n.content for n in neurons.values() if n.content]
        original_token_count = sum(_token_count(nc) for nc in neuron_contents)
        
        backup_created = False
        entities_preserved = len([n for n in neurons.values() 
                                 if n.type in ("entity", "concept", "eternal")])
        
        if target_tier == InfinityTier.COLD:
            # COLD: Keep only core neurons + core synapses
            keep_neurons = {
                nid for nid, n in neurons.items()
                if n.type in self._config.cold_keep_neuron_types
            }
            
            # Filter synapses
            keep_synapses: set[str] = set()
            for syn_id in fiber.synapse_ids:
                syn = await self._storage.get_synapse(syn_id)
                if syn and str(syn.type) in self._config.cold_keep_synapse_types:
                    keep_synapses.add(syn_id)
            
            # Create summary from remaining content
            summary_text = " ".join(
                neurons[nid].content for nid in keep_neurons
                if nid in neurons and neurons[nid].content
            )
            
            compressed_token_count = _token_count(summary_text)
            
            if not dry_run:
                # Update fiber
                updated_fiber = dc_replace(
                    fiber,
                    neuron_ids=keep_neurons,
                    synapse_ids=keep_synapses,
                    summary=summary_text[:500] if len(summary_text) > 500 else summary_text,
                    compression_tier=int(InfinityTier.COLD),
                )
                await self._storage.update_fiber(updated_fiber)
        
        elif target_tier == InfinityTier.WARM:
            # WARM: Create summary, keep ENTITY + CONCEPT + ETERNAL + ANCHOR
            keep_neurons = {
                nid for nid, n in neurons.items()
                if n.type in self._config.warm_keep_neuron_types
            }
            
            # Generate summary from all content
            full_content = " ".join(neuron_contents)
            # Simple extractive summary: first 3 sentences
            sentences = split_sentences(full_content)
            summary_sentences = sentences[:3] if len(sentences) >= 3 else sentences
            summary_text = " ".join(summary_sentences)
            
            compressed_token_count = _token_count(summary_text) + sum(
                _token_count(neurons[nid].content)
                for nid in keep_neurons
                if nid in neurons and neurons[nid].content
            )
            
            if not dry_run:
                # Backup original (reversible)
                if not fiber.summary:
                    backup_created = True
                    await self._storage.save_compression_backup(fiber.id, full_content)
                
                # Mark compressed neurons (ACTION, STATE, SENSORY)
                for nid, neuron in neurons.items():
                    if neuron.type in ("action", "state", "sensory"):
                        # Mark as compressed in metadata (original content saved in backup)
                        updated_neuron = neuron.with_metadata(compressed=True)
                        await self._storage.update_neuron(updated_neuron)
                
                # Update fiber
                updated_fiber = dc_replace(
                    fiber,
                    summary=summary_text,
                    compression_tier=int(InfinityTier.WARM),
                )
                await self._storage.update_fiber(updated_fiber)
    ...
    async def promote_fiber(
        self,
        fiber: Fiber,
        *,
        dry_run: bool = False,
    ) -> bool:
        """Promote fiber after recall (increase tier priority).
        
        InfinityNeural: Recall = thÄƒng tier (2â†’1â†’0). Also increments frequency.
        
        Args:
            fiber: Fiber to promote
            dry_run: If True, don't apply changes
            
        Returns:
            True if promoted, False if already at top tier
        """
        
        current_tier = InfinityTier(fiber.compression_tier)
        
        if current_tier == InfinityTier.HOT:
            # Already at top, just increment frequency
            if not dry_run:
                updated = dc_replace(fiber, frequency=fiber.frequency + 1)
                await self._storage.update_fiber(updated)
            return False
        
        if not dry_run:
            # Promote: tier goes down (2â†’1 or 1â†’0)
            new_tier = InfinityTier(max(0, int(current_tier) - 1))
            updated = dc_replace(
                fiber,
                compression_tier=int(new_tier),
                frequency=fiber.frequency + 1,
            )
            await self._storage.update_fiber(updated)
